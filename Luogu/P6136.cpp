...
int op, x, n, lastans, m, ans;
int main(){
	n = read(); m = read();
    for(int i = 1; i <= n; i++) tr.insert(read());
	while(m--){
//		tr.insert(-inf); tr.insert(inf);
		op = read(); x = read(); x ^= lastans;
		if(op == 1) tr.insert(x);
		if(op == 2) tr.del(x);
		if(op == 3) ans ^= (lastans = tr.rank(x));
		if(op == 4) ans ^= (lastans = tr.kth(x));
		if(op == 5) ans ^= (lastans = tr.pre(x));
		if(op == 6) ans ^= (lastans = tr.nxt(x));
	}
    write(ans);
} 