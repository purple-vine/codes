...
#include <vector>
using namespace std;
const int M = 2000005, p = 167772161;
inline int read() { 
...
int n, m;
int f[M], g[M], fac[M], invfac[M], ans[M];
int main(){
	scanf("%d", &n);
    fac[0] = invfac[0] = 1;
    for(int i = 1; i <= n; i++)
    ...
    for(int i = 0; i <= n; i++) f[i] = 1ll * (i & 1 ? -1 : 1) * invfac[i] % p;
    for(int i = 0; i <= n; i++) g[i] = 1ll * qpow(i, n) * invfac[i] % p;
    polymul(f, g, n, n+1);
    print(f, n+1);
}
/*
